...
use crate::stations::{binding_counts, FPS_STATIONS, VIEWBOX_H, VIEWBOX_W};
...
pub fn show(app: &mut YokeApp, ui: &mut egui::Ui) {
    ...
    let to_screen = |x: f32, y: f32| Pos2::new(x.mul_add(sx, rect.left()), y.mul_add(sy, rect.top()));
    ...
    for st in FPS_STATIONS {
        ...
        let stroke_color = if selected { palette.accent } else { palette.line };
        ...
        painter.text(
            ...
            if selected { palette.accent } else { palette.ink_2 },
        );
        if resp.clicked() {
            clicked = Some(st.id);
        }
    ...
    if let Some(id) = clicked {
        ...
        let next = if app.selected_station() == Some(id) { None } else { Some(id) };
