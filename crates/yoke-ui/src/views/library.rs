...
use crate::state::{AppState, OpenProfile, ProfileSource, use_state};
...
fn CommunityProfileList(state: AppState) -> impl IntoView {
    let community_profiles = state.community_profiles;
    view! {
        <div class="qs-section">
            <h3>"Community"</h3>
            <Show
                when=move || !community_profiles.get().is_empty()
                fallback=|| view! { <p class="qs-muted">"Loading..."</p> }
            >
                {
                    let state = state.clone();
                    move || {
                        ...
                        view! {
                            <ul class="qs-list">
                                <For
                                    each=move || community_profiles.get()
